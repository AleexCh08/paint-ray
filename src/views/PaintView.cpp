...
PaintView::PaintView(PaintDocument* document) : m_document(document) {
    // Aumentamos ligeramente el tamaño de la letra para un look más técnico
...
void PaintView::Render() {
    ...
    // --- 1. RENDERIZADO DEL MODELO (Canvas) ---
    ...
    // Panel Superior (Acciones Globales)
    ...
    for (int i = 0; i < 6; i++) {
        ...
        // Resaltado visual manual del botón activo (Estilo profesional)
        if (isActive) {
            DrawRectangleRec({btnRect.x - 2, btnRect.y - 2, btnRect.width + 4, btnRect.height + 4}, DARKGRAY);
        }

        // Usamos GuiButton, que es infalible al detectar clics
        if (GuiButton(btnRect, toolNames[i])) {
            m_document->SetCurrentTool((ShapeType)i);
        }
    }

    // Selector de Color (Integrado en el panel izquierdo)
    ...
    for(int i = 0; i < 6; i++) {
        ...
        // Borde resaltado si es el color seleccionado
        ...
        // Pintar el cuadro con el color real
